...
FuelGauge::FuelGauge(IGaugeMotor& motor, IClock& clock, const Config& cfg)
...
bool FuelGauge::supports(InstrumentSignalType type) const {
  return type == InstrumentSignalType::FuelLevel;
}

void FuelGauge::apply(const InstrumentSignal& signal) {
  if (signal.type == InstrumentSignalType::FuelLevel) {
    setFuelLevel(signal.value);
  }
}
