...
RpmGauge::RpmGauge(IGaugeMotor& motor, IClock& clock, const Config& cfg)
...
bool RpmGauge::supports(InstrumentSignalType type) const {
  return type == InstrumentSignalType::Rpm;
}

void RpmGauge::apply(const InstrumentSignal& signal) {
  if (signal.type == InstrumentSignalType::Rpm) {
    setRpm(signal.value);
  }
}
