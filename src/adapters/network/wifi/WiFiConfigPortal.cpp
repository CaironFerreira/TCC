...
static void copyText(char* buffer, size_t bufferSize, const char* value) {
...
void WiFiConfigPortal::copyLocalIp(char* buffer, size_t bufferSize) const {
  if (WiFi.status() == WL_CONNECTED) {
    const String ip = WiFi.localIP().toString();
    copyText(buffer, bufferSize, ip.c_str());
    return;
  }
  ...
  copyText(buffer, bufferSize, "0.0.0.0");
}

void WiFiConfigPortal::copyConnectedSsid(char* buffer, size_t bufferSize) const {
  const String ssid = WiFi.SSID();
  copyText(buffer, bufferSize, ssid.c_str());
}
